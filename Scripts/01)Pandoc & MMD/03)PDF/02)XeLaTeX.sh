...
cd "$(dirname "$BB_DOC_PATH")" && /Library/TeX/texbin/xelatex "$BB_DOC_PATH" -o "${BB_DOC_PATH%%.*}".pdf && /Library/TeX/texbin/biber "${BB_DOC_PATH%%.*}" && /Library/TeX/texbin/makeindex "${BB_DOC_PATH%%.*}".idx && /Library/TeX/texbin/xelatex "${BB_DOC_PATH%%.*}".tex -o "${BB_DOC_PATH%%.*}".pdf && /Library/TeX/texbin/xelatex "${BB_DOC_PATH%%.*}".tex -o "${BB_DOC_PATH%%.*}".pdf && open "${BB_DOC_PATH%%.*}".pdf
